...
export async function generateInvoiceDocument(input: GenerateInvoiceInput) {
  ...
  const lineItems: InvoiceLineItemRow[] = lpo.lineItems.map((line) => ({
  ...
  // Atomic, race-safe: two people generating an invoice at the same moment
  // still get distinct sequence numbers (Postgres upsert under the hood).
